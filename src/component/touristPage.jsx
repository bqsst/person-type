...
function TouristPage() {
    ...
    const buttonClick = () => {
        if (selectedIndex !== null) {
            navigate('/component/bedroomPage', {
                state: { selectedTourist: selectedIndex, selectedColor: locationFromRouter.state.selectedColor }
            });
        }
    }
    ...
    const thisPageData = dataImage.slice(6, 12);

    const touristElement = thisPageData.map((image, index) => {
    ...
    return (
        <div className="flex flex-col h-screen w-full">
            <div className='flex justify-normal items-center h-1/6 w-full pt-6'>
                <h1 className='text-blue-950 text-3xl md:text-5xl font-bold ml-[120px] sm:ml-[240px] md:ml-[80px] sm:mb-[20px] lg:ml-32 lg:mt-[20px]'>สถานที่ท่องเที่ยว</h1>
            </div>
